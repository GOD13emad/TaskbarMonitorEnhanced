...
namespace TaskbarMonitorEnhanced
{
    ...
    internal sealed class CpuTemperatureReader : IDisposable
    {
        ...
        private CpuTemperatureSample ReadElevatedBroker()
        {
            ...
            try
            {
                ...
                string json=File.ReadAllText(AppPaths.CpuTempBrokerData,Encoding.UTF8);
                JavaScriptSerializer js=new JavaScriptSerializer();
    ...
    internal sealed class MetricsEngine : IDisposable
    {
        ...
        {
            if(memoryModules.Count>0&&(DateTime.UtcNow-lastMemoryModuleQuery).TotalSeconds<60)
            {
        ...
        {
            if(diskTopology.Count>0&&(DateTime.UtcNow-lastDiskTopologyQuery).TotalSeconds<60)return;
