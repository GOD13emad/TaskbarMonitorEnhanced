    ...
    internal static class Program
    {
            ...
            try
            {
                ...
                foreach(IHardware hw in computer.Hardware)
                {
                    ...
                    foreach(ISensor sensor in hw.Sensors)
                    {
                        ...
                        if(Valid(sensor.Value))
                            temps.Add(Tuple.Create(sensor.Name,sensor.Value.Value));
                    }
                    ...
                    foreach(IHardware sub in hw.SubHardware)
                    {
                        foreach(ISensor sensor in sub.Sensors)
                        {
                            ...
                            if(Valid(sensor.Value))
                                temps.Add(Tuple.Create(sub.Name+"/"+sensor.Name,sensor.Value.Value));
                        }
                    }
                }
                ...
                Tuple<string,float> chosen=temps
                    .OrderByDescending(x=>Priority(x.Item1))
                    .ThenByDescending(x=>x.Item2)
                    .First();

                List<float> core=temps
        ...
        private static void AtomicWrite(string path,TempRecord r)
        {
            ...
            string json=js.Serialize(new Dictionary<string,object> {
                ...
                {"CpuName",r.CpuName??""},
                {"Error",r.Error??""},
                {"Is64BitProcess",r.Is64BitProcess},
                {"IsElevated",r.IsElevated},
                {"CpuTemperatureSensors",r.CpuTemperatureSensors.ToArray()},
                    ...
                    while(true)
                    {
                        ...
                        catch(Exception loopEx)
                        {
                            ...
                            if(consecutiveFailures>=5)
                            {
                                ...
                                {
                                    computer.Open();
                                    BrokerLog("BROKER_COMPUTER_REOPEN_PASS");
                                }
                                catch(Exception reopenEx)
                                {
                                    BrokerLog("BROKER_COMPUTER_REOPEN_FAIL "+reopenEx.ToString());
                                }
                                consecutiveFailures=0;
                            }
                        }
